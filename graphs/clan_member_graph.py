...
This module:
- Loads monthly clan member coc-data from a GitHub-hosted JSON source
...
class ClanMemberGraph:
    ...
    Handles coc-data loading, preprocessing, and visualization generation
    for current clan members.

    The class supports:
    - Dynamic month-based coc-data loading
    - Separation of war participation status (IN / OUT)
    - Generation of multiple chart types from the same dataset
    - Graceful fallback to latest available coc-data when requested coc-data is missing
    """
    ...
    def __init__(self):
        ...
        - Data source URL
        - Loaded DataFrame
        - Informational message for missing or fallback coc-data
        """
    ...
    def update_and_load_data(self, month_year):
        """
        Load and preprocess clan member coc-data for a given month.

        This method:
        - Dynamically builds the coc-data source URL
        - Fetches JSON coc-data from GitHub
        - Falls back to the latest available month if coc-data is missing
    ...
    def create_bar_graphs(self):
        ...
        fig1 = go.Figure()
        fig1.add_trace(
            go.Bar(x=self.df_in["name"], y=self.df_in["warattack"], name="War IN")
        )
        fig1.add_trace(
            go.Bar(x=self.df_out["name"], y=self.df_out["warattack"], name="War OUT")
        )
        fig1.update_layout(
            barmode="group",
            title="Vertical Bar Chart of War Attack",
            xaxis_title="Name",
        ...
        fig2 = go.Figure()
        fig2.add_trace(
            go.Bar(x=self.df_in["name"], y=self.df_in["clancapital"], name="War IN")
        )
        fig2.add_trace(
            go.Bar(x=self.df_out["name"], y=self.df_out["clancapital"], name="War OUT")
        )
        fig2.update_layout(
            barmode="group",
            title="Vertical Bar Chart of Clan Capital",
            xaxis_title="Name",
        ...
        fig3 = go.Figure()
        fig3.add_trace(
            go.Bar(x=self.df_in["name"], y=self.df_in["clangames"], name="War IN")
        )
        fig3.add_trace(
            go.Bar(x=self.df_out["name"], y=self.df_out["clangames"], name="War OUT")
        )
        fig3.update_layout(
            barmode="group",
            title="Vertical Bar Chart of Clan Games",
            xaxis_title="Name",
        ...
        fig4 = go.Figure()
        fig4.add_trace(
            go.Bar(x=self.df_in["name"], y=self.df_in["clangamesmaxed"], name="War IN")
        )
        fig4.add_trace(
            go.Bar(
                x=self.df_out["name"], y=self.df_out["clangamesmaxed"], name="War OUT"
            )
        )
        fig4.update_layout(
            barmode="group",
            title="Vertical Bar Chart of Clan Games Maxed",
            xaxis_title="Name",
        ...
        fig5 = go.Figure()
        fig5.add_trace(
            go.Bar(x=self.df_in["name"], y=self.df_in["clanscore"], name="War IN")
        )
        fig5.add_trace(
            go.Bar(x=self.df_out["name"], y=self.df_out["clanscore"], name="War OUT")
        )
        fig5.update_layout(
            barmode="group",
            title="Vertical Bar Chart of Clan Score",
            xaxis_title="Name",
        ...
        for column in self.numerical_df.columns:
            if column not in ["srno"]:  # Exclude 'srno'
                fig6.add_trace(
                    go.Bar(name=column, x=self.df["name"], y=self.df[column])
                )
        fig6.update_layout(
            barmode="stack",
            title="Stacked Bar Chart of Numerical Values",
            xaxis_title="Names",
            yaxis_title="Values",
        )
        ...
        fig7.add_trace(
            go.Bar(name="War Status", x=war_counts.index, y=war_counts.values)
        )
        status_counts = self.df["status"].value_counts()
        fig7.add_trace(
            go.Bar(name="Status Count", x=status_counts.index, y=status_counts.values)
        )
        fig7.update_layout(
            barmode="stack",
            title="Count of War Status and Status Count",
    ...
    def create_pie_charts(self):
        ...
        - Binned distributions for war attacks, clan capital, clan games,
          clan games maxed and clan score
        ...
        # 2. War Participation Distribution
        war_status = self.df["war"].value_counts()
        fig2 = go.Figure(
            data=[go.Pie(labels=war_status.index, values=war_status.values)]
        )
        fig2.update_layout(title_text="War Participation Distribution")

        # 3. War Attacks Distribution
        ...
        fig4 = px.pie(
            self.df, names="clancapital_range", title="Clan Capital Distribution"
        )

        # 5. Clan Games Distribution
        ...
        fig5 = px.pie(
            self.df, names="clangames_range", title="Clan Games Distribution"
        )

        # 6. Clan Games Maxed Distribution
        maxed_games = self.df["clangamesmaxed"].value_counts()
        fig6 = go.Figure(
            data=[go.Pie(labels=maxed_games.index, values=maxed_games.values)]
        )
        fig6.update_layout(title_text="Clan Games Maxed Distribution")

        # 7. Clan Score Distribution
        self.df["clanscore_range"] = pd.cut(
            self.df["clanscore"],
            bins=[-1, 100, 300, 500, 700, float("inf")],
            labels=["0-100", "101-300", "301-500", "501-700", "700+"],
        )
        fig7 = px.pie(self.df, names="clanscore_range", title="Clan Score Distribution")
    ...
    def create_line_charts(self):
        ...
        for column in numerical_columns:
            ...
            fig.update_layout(
                title=f"Line Chart of {column} by Name",
                xaxis_title="Name",
                yaxis_title=column,
            )
        ...
        fig_combined.update_layout(
            title="Line Chart of Multiple Metrics by Name",
            xaxis_title="Name",
            yaxis_title="Values",
        )
    ...
    def create_heatmaps(self):
        ...
        Includes:
        - Pairwise numerical density heatmaps
        ...
        # 2. Categorical heatmap: War vs Player Status
        if "war" in self.df.columns and "status" in self.df.columns:
            war_status_counts = pd.crosstab(
                self.df["war"],
                self.df["status"]
            )
    ...
    def create_treemaps(self):
        ...
        # 1. Treemap based on 'war' and 'warattack'
        ...
        if not df_filtered.empty:
            treemap1 = px.treemap(
                ...
                path=["war", "name"],
                values="warattack",
                title="Treemap of War Attack by War Status",
        ...
        # 2. Treemap based on 'status' and 'clancapital'
        ...
        if not df_filtered.empty:
            treemap2 = px.treemap(
                ...
                path=["status", "name"],
                values="clancapital",
                title="Treemap of Clan Capital by Player Status",
        ...
        # 3. Treemap based on 'status' and 'clangames'
        ...
        if not df_filtered.empty:
            treemap3 = px.treemap(
                ...
                path=["status", "name"],
                values="clangames",
                title="Treemap of Clan Games by Player Status",
        ...
        # 4. Treemap based on 'status' and 'clangamesmaxed'
        ...
        if not df_filtered.empty:
            treemap4 = px.treemap(
                ...
                title="Treemap of Clan Games Maxed by Player Status",
            ...
            # 5. Treemap based on 'status' and 'clanscore'
    ...
    def create_sunburst_charts(self):
        ...
        # 1. Sunburst chart based on 'war' and 'warattack'
        df_filtered = self.df[
            self.df["warattack"].notna() & (self.df["warattack"] != 0)
        ]
        if not df_filtered.empty:
            sunburst1 = px.sunburst(
                ...
                path=["war", "name"],
                values="warattack",
                title="Sunburst Chart of War Attack by War Status",
                labels={"warattack": "War Attack", "name": "Player Name"},
                color="warattack",
                hover_data=["warattack"],
                color_continuous_scale="Viridis",
        ...
        # 2. Sunburst chart based on 'status' and 'clancapital'
        df_filtered = self.df[
            self.df["clancapital"].notna() & (self.df["clancapital"] != 0)
        ]
        if not df_filtered.empty:
            sunburst2 = px.sunburst(
                ...
                path=["status", "name"],
                values="clancapital",
                title="Sunburst Chart of Clan Capital by Player Status",
                labels={"clancapital": "Clan Capital", "name": "Player Name"},
                color="clancapital",
                hover_data=["clancapital"],
                color_continuous_scale="Viridis",
        ...
        # 3. Sunburst chart based on 'status' and 'clangames'
        df_filtered = self.df[
            self.df["clangames"].notna() & (self.df["clangames"] != 0)
        ]
        if not df_filtered.empty:
            sunburst3 = px.sunburst(
                ...
                path=["status", "name"],
                values="clangames",
                title="Sunburst Chart of Clan Games by Player Status",
                labels={"clangames": "Clan Games", "name": "Player Name"},
                color="clangames",
                hover_data=["clangames"],
                color_continuous_scale="Viridis",
        ...
        # 4. Sunburst chart based on 'status' and 'clangamesmaxed'
        df_filtered = self.df[
            self.df["clangamesmaxed"].notna() & (self.df["clangamesmaxed"] != 0)
        ]
        if not df_filtered.empty:
            sunburst4 = px.sunburst(
                ...
                path=["status", "name"],
                values="clangamesmaxed",
                title="Sunburst Chart of Clan Games Maxed by Player Status",
                labels={"clangamesmaxed": "Clan Games Maxed", "name": "Player Name"},
                color="clangamesmaxed",
                hover_data=["clangamesmaxed"],
                color_continuous_scale="Viridis",
            ...
            # 5. Sunburst chart based on 'status' and 'clanscore'
            df_filtered = self.df[
                self.df["clanscore"].notna() & (self.df["clanscore"] != 0)
                ]
            if not df_filtered.empty:
                sunburst5 = px.sunburst(
                    ...
                    path=["status", "name"],
                    values="clanscore",
                    title="Sunburst Chart of Clan Score by Player Status",
                    labels={"clanscore": "Clan Score", "name": "Player Name"},
                    color="clanscore",
                    hover_data=["clanscore"],
                    color_continuous_scale="Viridis",
    ...
    def create_density_plots(self):
        """
        Generate density contour plots for pairwise numerical comparisons.

        Includes:
        - Pairwise numerical density contour plots
        ...
        # Get numerical columns, excluding srno
        numerical_columns = self.numerical_df.columns.drop(
            "srno",
            errors="ignore"
        )

        # Only use columns that exist in self.df
        numerical_columns = [
            col for col in numerical_columns
            if col in self.df.columns
        ]

        # Generate density plot for every numerical pair
        for col1, col2 in combinations(numerical_columns, 2):
            fig = go.Figure()
    ...
    def create_area_graphs(self):
        ...
        # 1. Area Chart of War Attack Over Players (War IN/OUT)
        fig1 = go.Figure()
        fig1.add_trace(
            go.Scatter(
                x=self.df_in["name"],
                y=self.df_in["warattack"],
                fill="tozeroy",
                name="War IN",
            )
        )
        fig1.add_trace(
            go.Scatter(
                x=self.df_out["name"],
                y=self.df_out["warattack"],
                fill="tozeroy",
                name="War OUT",
            )
        )
        fig1.update_layout(
            title="Area Chart of War Attack Over Players (War IN/OUT)",
            xaxis_title="Name",
            yaxis_title="War Attack",
        )

        # 2. Area Chart of Clan Capital Over Players (War IN/OUT)
        fig2 = go.Figure()
        fig2.add_trace(
            go.Scatter(
                x=self.df_in["name"],
                y=self.df_in["clancapital"],
                fill="tozeroy",
                name="War IN",
            )
        )
        fig2.add_trace(
            go.Scatter(
                x=self.df_out["name"],
                y=self.df_out["clancapital"],
                fill="tozeroy",
                name="War OUT",
            )
        )
        fig2.update_layout(
            title="Area Chart of Clan Capital Over Players (War IN/OUT)",
            xaxis_title="Name",
            yaxis_title="Clan Capital",
        )

        # 3. Area Chart of Clan Games Over Players (War IN/OUT)
        fig3 = go.Figure()
        fig3.add_trace(
            go.Scatter(
                x=self.df_in["name"],
                y=self.df_in["clangames"],
                fill="tozeroy",
                name="War IN",
            )
        )
        fig3.add_trace(
            go.Scatter(
                x=self.df_out["name"],
                y=self.df_out["clangames"],
                fill="tozeroy",
                name="War OUT",
            )
        )
        fig3.update_layout(
            title="Area Chart of Clan Games Over Players (War IN/OUT)",
            xaxis_title="Name",
            yaxis_title="Clan Games",
        )

        # 4. Area Chart of Clan Games Maxed Over Players (War IN/OUT)
        fig4 = go.Figure()
        fig4.add_trace(
            go.Scatter(
                x=self.df_in["name"],
                y=self.df_in["clangamesmaxed"],
                fill="tozeroy",
                name="War IN",
            )
        )
        fig4.add_trace(
            go.Scatter(
                x=self.df_out["name"],
                y=self.df_out["clangamesmaxed"],
                fill="tozeroy",
                name="War OUT",
            )
        )
        fig4.update_layout(
            title="Area Chart of Clan Games Maxed Over Players (War IN/OUT)",
            xaxis_title="Name",
            yaxis_title="Clan Games Maxed",
        )

        # 5. Area Chart of Clan Score Over Players (War IN/OUT)
        fig5 = go.Figure()
        fig5.add_trace(
            go.Scatter(
                x=self.df_in["name"],
                y=self.df_in["clanscore"],
                fill="tozeroy",
                name="War IN",
            )
        )
        fig5.add_trace(
            go.Scatter(
                x=self.df_out["name"],
                y=self.df_out["clanscore"],
                fill="tozeroy",
                name="War OUT",
            )
        )
        fig5.update_layout(
            title="Area Chart of Clan Score Over Players (War IN/OUT)",
            xaxis_title="Name",
            yaxis_title="Clan Score",
        )
        ...
        fig6.update_layout(
            title="Stacked Area Chart of Numerical Values",
            xaxis_title="Name",
            yaxis_title="Values",
        )
    ...
    def create_polar_charts(self):
        ...
        # 1. Polar Chart of War Attack Over Players (War IN/OUT)
        fig1 = go.Figure()
        fig1.add_trace(
            go.Scatterpolar(
                r=self.df_in["warattack"],
                theta=self.df_in["name"],
                fill="toself",
                name="War IN",
            )
        )
        fig1.add_trace(
            go.Scatterpolar(
                r=self.df_out["warattack"],
                theta=self.df_out["name"],
                fill="toself",
                name="War OUT",
            )
        )
        fig1.update_layout(
            title="Polar Chart of War Attack Over Players (War IN/OUT)",
            polar=dict(radialaxis=dict(visible=True)),
        )

        # 2. Polar Chart of Clan Capital Over Players (War IN/OUT)
        fig2 = go.Figure()
        fig2.add_trace(
            go.Scatterpolar(
                r=self.df_in["clancapital"],
                theta=self.df_in["name"],
                fill="toself",
                name="War IN",
            )
        ...
        fig2.update_layout(
            title="Polar Chart of Clan Capital Over Players (War IN/OUT)",
            polar=dict(radialaxis=dict(visible=True)),
        )

        # 3. Polar Chart of Clan Games Over Players (War IN/OUT)
        fig3 = go.Figure()
        fig3.add_trace(
            go.Scatterpolar(
                r=self.df_in["clangames"],
                theta=self.df_in["name"],
                fill="toself",
                name="War IN",
            )
        ...
        fig3.update_layout(
            title="Polar Chart of Clan Games Over Players (War IN/OUT)",
            polar=dict(radialaxis=dict(visible=True)),
        )

        # 4. Polar Chart of Clan Games Maxed Over Players (War IN/OUT)
        fig4 = go.Figure()
        fig4.add_trace(
            go.Scatterpolar(
                r=self.df_in["clangamesmaxed"],
                theta=self.df_in["name"],
                fill="toself",
                name="War IN",
            )
        ...
        fig4.update_layout(
            title="Polar Chart of Clan Games Maxed Over Players (War IN/OUT)",
            polar=dict(radialaxis=dict(visible=True)),
        )

        # 5. Polar Chart of Clan Score Over Players (War IN/OUT)
        fig5 = go.Figure()
        fig5.add_trace(
            go.Scatterpolar(
                r=self.df_in["clanscore"],
                theta=self.df_in["name"],
                fill="toself",
                name="War IN",
            )
        ...
        fig5.update_layout(
            title="Polar Chart of Clan Score Over Players (War IN/OUT)",
            polar=dict(radialaxis=dict(visible=True)),
        )
        ...
        fig6.update_layout(
            title="Stacked Polar Chart of Numerical Values",
            polar=dict(radialaxis=dict(visible=True)),
        )
    ...
    def create_waterfall_charts(self):
        ...
        fig6.update_layout(
            title="Stacked Waterfall Chart of Numerical Values",
            xaxis_title="Name",
            yaxis_title="Values",
        )
